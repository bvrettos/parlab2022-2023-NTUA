...
int main(int argc, char **argv)
{
    ...
    MPI_Status status;                       // Needed for SendRecv functions
...
#define T 256
        for (t = 0; t < T; t++)
        {
            ...
            if (west != MPI_PROC_NULL)
                MPI_Sendrecv(&u_previous[1][1], 1, mpi_column, west, 420, &u_previous[1][0], 1, mpi_column, west, 420, MPI_COMM_WORLD, &status);

            if (east != MPI_PROC_NULL)
                MPI_Sendrecv(&u_previous[1][local[1]], 1, mpi_column, east, 420, &u_previous[1][local[1] + 1], 1, mpi_column, east, 420, MPI_COMM_WORLD, &status);

            if (north != MPI_PROC_NULL)
                MPI_Sendrecv(&u_previous[1][1], local[1], MPI_DOUBLE, north, 420, &u_previous[0][1], local[1], MPI_DOUBLE, north, 420, MPI_COMM_WORLD, &status);

            if (south != MPI_PROC_NULL)
                MPI_Sendrecv(&u_previous[local[0]][1], local[1], MPI_DOUBLE, south, 420, &u_previous[local[0] + 1][1], local[1], MPI_DOUBLE, south, 420, MPI_COMM_WORLD, &status);
            ...
            for (i = i_min; i < i_max; i++)
                for (j = j_min; j < j_max; j++)
                    if ((i+j)%2==0)
                        u_current[i][j] = u_previous[i][j] + omega * (u_previous[i-1][j] + u_previous[i+1][j] + u_previous[i][j-1] + u_previous[i][j+1] - 4.0*u_previous[i][j]) / 4.0;
            ...
            if (west != MPI_PROC_NULL)
                MPI_Sendrecv(&u_current[1][1], 1, mpi_column, west, 420, &u_current[1][0], 1, mpi_column, west, 420, MPI_COMM_WORLD, &status);

            if (east != MPI_PROC_NULL)
                MPI_Sendrecv(&u_current[1][local[1]], 1, mpi_column, east, 420, &u_current[1][local[1] + 1], 1, mpi_column, east, 420, MPI_COMM_WORLD, &status);

            if (north != MPI_PROC_NULL)
                MPI_Sendrecv(&u_current[1][1], local[1], MPI_DOUBLE, north, 420, &u_current[0][1], local[1], MPI_DOUBLE, north, 420, MPI_COMM_WORLD, &status);

            if (south != MPI_PROC_NULL)
                MPI_Sendrecv(&u_current[local[0]][1], local[1], MPI_DOUBLE, south, 420, &u_current[local[0] + 1][1], local[1], MPI_DOUBLE, south, 420, MPI_COMM_WORLD, &status);
            ...
            for (i = i_min; i < i_max; i++)
                for (j = j_min; j < j_max; j++)
                    if ((i + j) % 2 == 1)
                        u_current[i][j] = u_previous[i][j] + omega * (u_current[i - 1][j] + u_current[i + 1][j] + u_current[i][j - 1] + u_current[i][j + 1] - 4.0*u_previous[i][j]) / 4.0;
        ...
        if (rank == 0)
        {
            printf("RedBlackSOR X %d Y %d Px %d Py %d Iter %d CommunicationTime(Median) %lf ComputationTime(Max) %lf ConverganceTime(Max) %lf TotalTime %lf midpoint %lf\n", global[0], global[1], grid[0], grid[1], t - 1, comm_time, comp_time, conv_time, total_time, U[global[0] / 2][global[1] / 2]);
