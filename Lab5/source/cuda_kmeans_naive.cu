                    ...
                    int    clusterId)
{
    ...
    float ans=0.0;
    float coord1, coord2;
    long objectsIndex = objectId * numCoords;
    long clustersIndex = clusterId * numCoords;

    for (i = 0; i < numCoords; i++){
        coord1 = objects[objectsIndex + i];
        coord2 = clusters[clustersIndex + i];
						...
						int blockSize)
{
	...
	fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf,%lf,%lf\n", "Naive", blockSize, timing/loop, timer_min, timer_max,1000*cpu_total_time,1000*transfer_total_time,1000*gpu_total_time);
	fclose(fp); 
